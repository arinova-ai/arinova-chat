...
// ---------------------------------------------------------------------------
// Draggable FAB for game overlay
// ---------------------------------------------------------------------------

type GameMode = "fullscreen" | "pip";
...
function GameFAB({
  ...
  // Initialise position (bottom-right with margin)
  useEffect(() => {
  ...
  if (pos.x < 0) return null; // not initialised yet
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => { if (!didDrag.current) onToggleMode(); }}
...
function GameDetailContent({ id }: { id: string }) {
  ...
  const [gameMode, setGameMode] = useState<GameMode | null>(null); // null = closed
  ...
  return (
    <div className="app-dvh flex bg-background">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-4">
                    <Button
                      ...
                      onClick={() => game.iframeUrl && setGameMode("fullscreen")}
                    >
      ...
      {/* Game iframe overlay — fullscreen or PIP */}
      {gameMode && game.iframeUrl && (
        <>
          <div
            className={
              gameMode === "fullscreen"
          ...
          <GameFAB
            mode={gameMode}
            onToggleMode={() =>
              setGameMode((m) => (m === "fullscreen" ? "pip" : "fullscreen"))
            }
