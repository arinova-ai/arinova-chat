...
export default function AdminDataRequestsPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-xl font-bold">{t("admin.dataRequests.title")}</h2>
      <p className="text-sm text-muted-foreground">{t("admin.dataRequests.subtitle")}</p>
          ...
          {requests.map((r) => (
            <div key={r.id} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{t("admin.dataRequests.type")} {r.type} · {r.createdAt}</p>
              </div>
              <Button size="sm" onClick={() => handleApprove(r.id)} disabled={approving === r.id}>
                {approving === r.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <><CheckCircle className="h-3.5 w-3.5 mr-1" />{t("admin.dataRequests.approve")}</>}
              </Button>
            </div>
          ))}
          {requests.length === 0 && <p className="text-center text-muted-foreground py-8">{t("admin.dataRequests.noRequests")}</p>}
        </div>
      )}
    </div>
  );
