...
export default function AdminRevenuePage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h2 className="text-xl font-bold">{t("admin.revenue.title")}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">{t("admin.revenue.totalTopups")}</p>
          <p className="text-2xl font-bold">{data?.totalRevenue ?? 0}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">{t("admin.revenue.totalPurchases")}</p>
          <p className="text-2xl font-bold">{data?.totalPurchases ?? 0}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">{t("admin.revenue.platformFees")}</p>
          <p className="text-2xl font-bold">{data?.platformFees ?? 0}</p>
        </div>
      </div>
      <div className="rounded-lg border border-border bg-card p-4">
        <h3 className="text-sm font-medium mb-4">{t("admin.revenue.trendChart")}</h3>
        ...
        <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 rounded bg-green-500" />{t("admin.revenue.topup")}</span>
          <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 rounded bg-orange-500" />{t("admin.revenue.spend")}</span>
        </div>
      </div>
    </div>
  );
