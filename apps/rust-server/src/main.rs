...
async fn main() {
    ...
    sqlx::query("ALTER TABLE conversations ADD COLUMN IF NOT EXISTS avatar_url TEXT").execute(&db).await.ok();
    sqlx::query("ALTER TABLE notebooks ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT FALSE").execute(&db).await.ok();
    ...
    // arinova-help skill content managed in DB — no longer overwritten on startup
