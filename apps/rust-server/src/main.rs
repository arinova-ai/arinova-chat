...
async fn main() {
    ...
    // Update arinova-help skill prompt_content with full CLI docs
    sqlx::query(r#"UPDATE skills SET prompt_content = E'# Arinova Chat CLI Reference\n\n## Global Options\n- `--agent <name>` — Select account by name from openclaw config\n- `--token <botToken>` — Use bot token directly (highest priority)\n\nPriority: --token > --agent > default account\n\n## Commands\n\n### Messages\n- `openclaw arinova message send --conversation-id <id> --content <text> [--reply-to <id>]`\n- `openclaw arinova message list --conversation-id <id> [--limit <n>] [--cursor <id>]`\n\n### Files\n- `openclaw arinova file upload --conversation-id <id> --file-path <path>`\n  Returns { url: \"https://...\" }. Use URL in message content: `![image](url)`\n\n### Notes\n- `openclaw arinova note list [--notebook-id <id>]`\n- `openclaw arinova note create --notebook-id <id> --title <title> [--content <text>] [--tags <tags>]`\n- `openclaw arinova note update --note-id <id> [--title <text>] [--content <text>]`\n- `openclaw arinova note delete --note-id <id>`\n\n### Memory\n- `openclaw arinova memory query --query <text> [--limit <n>]`\n\n### Kanban\n- `openclaw arinova kanban board list|create|update|archive|unarchive`\n- `openclaw arinova kanban column list|create|update|delete|reorder`\n- `openclaw arinova kanban card list|create|update|complete|archive`\n- `openclaw arinova kanban card add-commit|commits|link-note|unlink-note|notes`\n- `openclaw arinova kanban label list|create|update|delete`\n- `openclaw arinova kanban card add-label|remove-label`\n\n## Common Examples\n\n### Send a message\n```\nopenclaw arinova message send --conversation-id abc-123 --content \"Hello!\"\n```\n\n### Upload image then send\n```\nopenclaw arinova file upload --conversation-id abc-123 --file-path ./screenshot.png\nopenclaw arinova message send --conversation-id abc-123 --content \"![img](https://...url...)\"\n```\n\n### List kanban cards\n```\nopenclaw arinova kanban card list\n```\n\n### Create a card\n```\nopenclaw arinova kanban card create --title \"Fix bug\" --column-name \"To Do\"\n```\n\n### Use specific token\n```\nopenclaw arinova --token ari_xxx kanban card list\n```\n\n## curl Fallback\nIf CLI is unavailable, use curl with bot token:\n```\ncurl -X POST https://api.chat.arinova.ai/api/agent/send \\\n  -H \"Authorization: Bearer <bot-token>\" \\\n  -H \"Content-Type: application/json\" \\\n  -d ''{ \"conversationId\": \"<id>\", \"content\": \"Hello\" }''\n```'
    WHERE slug = 'arinova-help'"#).execute(&db).await.ok();
