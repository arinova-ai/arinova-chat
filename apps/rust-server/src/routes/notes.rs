...
async fn get_related_capsules_inner(db: &PgPool, user_id: &str, content: &str, openai_key: Option<&str>) -> Vec<serde_json::Value> {
    ...
    let rows: Vec<(Uuid, String, f64, Uuid, String, Option<DateTime<Utc>>, Option<DateTime<Utc>>, f32, Option<Uuid>, Option<Uuid>)> = sqlx::query_as(
    ...
    rows.iter()
        .map(|(id, content, importance, capsule_id, capsule_name, source_start, source_end, _rank, source_conv_id, source_msg_id)| {
            json!({
                "id": id,
                "content": content,
...
async fn embedding_search(db: &PgPool, user_id: &str, content: &str, api_key: &str) -> Option<Vec<serde_json::Value>> {
    ...
    let rows: Vec<(Uuid, String, f64, Uuid, String, Option<DateTime<Utc>>, Option<DateTime<Utc>>, f64, Option<Uuid>, Option<Uuid>)> = sqlx::query_as(
        r#"SELECT me.id, me.content, me.importance, mc.id AS capsule_id, mc.name AS capsule_name,
                  me.source_start, me.source_end,
    ...
    let results: Vec<_> = rows.iter()
        .filter(|r| r.7 >= 0.3)
        .map(|(id, content, importance, capsule_id, capsule_name, source_start, source_end, similarity, source_conv_id, source_msg_id)| {
            json!({
                "id": id,
                "content": content,
                "importance": importance,
