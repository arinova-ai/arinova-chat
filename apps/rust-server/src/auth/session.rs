...
pub async fn validate_session(
    ...
    // Fetch user
    let user = sqlx::query_as::<_, (String, String, Option<String>, Option<String>, Option<String>, bool, bool)>(
        r#"SELECT email, name, image, bio, username, is_verified, banned FROM "user" WHERE id = $1"#,
    )
    .bind(&user_id)
    .fetch_optional(pool)
    .await?;
    ...
    match user {
        Some((email, name, image, bio, username, is_verified, banned)) => Ok(Some(ValidatedSession {
            user_id,
            email,
            name,
            image,
