...
export function registerCommunity(program: Command): void {
  ...
  community
    ...
    .action(async () => {
      try {
        const data = await get("/api/creator/community");
  ...
  community
    ...
    .action(async (opts: { name: string; type: string; description?: string }) => {
      try {
        const data = await post("/api/communities", {
          name: opts.name,
          type: opts.type,
  ...
  community
    ...
    .action(async (id: string, opts: { name?: string; description?: string }) => {
      try {
        ...
        const data = await patch(`/api/communities/${id}`, body);
        printResult(data);
      } catch (err) {
        printError(err);
      }
  ...
  community
    ...
    .action(async (communityId: string, agentId: string) => {
      try {
        const data = await post(`/api/communities/${communityId}/agents`, {
          agent_id: agentId,
        });
        printResult(data);
      } catch (err) {
  ...
  community
    ...
    .action(async (communityId: string, agentId: string) => {
      try {
        await del(`/api/communities/${communityId}/agents/${agentId}`);
  ...
  community
    ...
    .action(async (communityId: string) => {
      try {
        const data = await get(`/api/communities/${communityId}/members`);
        printResult(data);
      } catch (err) {
        printError(err);
      }
  ...
  community
    ...
    .action(async (communityId: string) => {
      try {
        const data = await get(`/api/communities/${communityId}/agents`);
        printResult(data);
      } catch (err) {
        printError(err);
      }
