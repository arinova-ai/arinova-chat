...
describe("message send parameters", () => {
  it("assembles correct URL and body for message send", () => {
    ...
    const url = `${account.apiUrl}/api/agent/send`;
    const body: Record<string, string> = { conversationId: opts.conversationId, content: opts.content };
    expect(url).toBe("https://api.test.arinova.ai/api/agent/send");
...
describe("kanban card list URL assembly", () => {
  it("builds correct URL without filters", () => {
    const url = `${MOCK_ACCOUNT.apiUrl}/api/agent/kanban/cards`;
    expect(url).toBe("https://api.test.arinova.ai/api/agent/kanban/cards");
  });

  it("builds correct URL for board list", () => {
    const url = `${MOCK_ACCOUNT.apiUrl}/api/agent/kanban/boards`;
    expect(url).toBe("https://api.test.arinova.ai/api/agent/kanban/boards");
  });
});
...
describe("message list URL assembly", () => {
  it("builds query string with limit and cursor", () => {
    ...
    const url = `${MOCK_ACCOUNT.apiUrl}/api/agent/conversations/conv-1/notes?${qs}`;
